-- ===========================================================
-- IAMED - SCRIPT DE SINCRONIZAÇÃO COMPLETO
-- Cole e execute no SQL Editor do Supabase
-- https://supabase.com/dashboard/project/kqfiwigggbdwwnzywhbx/editor
-- ===========================================================

-- ============================================================
-- PARTE 1: CORRIGIR TABELA `professionals` (coluna permissions)
-- ============================================================

ALTER TABLE professionals
  ADD COLUMN IF NOT EXISTS permissions TEXT[] DEFAULT '{}';

-- Atualizar permissions dos profissionais existentes
UPDATE professionals SET permissions = ARRAY['view_reception', 'view_agenda', 'view_hce', 'view_diagnostic', 'view_med_work', 'perform_prescribe']
  WHERE id = 'prof_1' AND (permissions IS NULL OR permissions = '{}');

UPDATE professionals SET permissions = ARRAY['view_reception', 'view_agenda', 'view_hce', 'view_diagnostic', 'view_finance', 'view_stock', 'view_med_work', 'view_crm', 'view_security', 'perform_admit', 'perform_prescribe', 'perform_sifen', 'perform_post_finance', 'perform_stock', 'perform_beds', 'perform_rbac']
  WHERE id = 'prof_2' AND (permissions IS NULL OR permissions = '{}');

UPDATE professionals SET permissions = ARRAY['view_agenda', 'view_hce', 'view_diagnostic', 'view_med_work', 'perform_prescribe']
  WHERE id = 'prof_3' AND (permissions IS NULL OR permissions = '{}');

UPDATE professionals SET permissions = ARRAY['view_reception', 'view_agenda', 'view_diagnostic', 'perform_admit', 'perform_beds']
  WHERE id = 'prof_4' AND (permissions IS NULL OR permissions = '{}');

UPDATE professionals SET permissions = ARRAY['view_agenda', 'view_hce']
  WHERE id = 'prof_5' AND (permissions IS NULL OR permissions = '{}');


-- ============================================================
-- PARTE 2: CORRIGIR CONSTRAINT DE STATUS EM `appointments`
-- ============================================================

ALTER TABLE appointments DROP CONSTRAINT IF EXISTS appointments_status_check;
ALTER TABLE appointments ADD CONSTRAINT appointments_status_check
  CHECK (status IN ('agendado', 'confirmado', 'pendente', 'cancelado', 'atendido',
                    'em sala de espera', 'em atendimento', 'finalizado', 'ausente', 'remarcado'));


-- ============================================================
-- PARTE 3: CRIAR TABELAS AUSENTES
-- ============================================================

-- 3.1 Lista de Espera Inteligente
...
DROP POLICY IF EXISTS "Allow public read waiting_list" ON waiting_list;
DROP POLICY IF EXISTS "Allow authenticated changes waiting_list" ON waiting_list;
...
-- 3.2 Bloqueios de Faixas Horárias
...
DROP POLICY IF EXISTS "Allow authenticated changes blocked_slots" ON blocked_slots;
...
DROP POLICY IF EXISTS "Allow public read whatsapp_reminders" ON whatsapp_reminders;
DROP POLICY IF EXISTS "Allow authenticated changes whatsapp_reminders" ON whatsapp_reminders;
...
DROP POLICY IF EXISTS "Allow public read call_center_logs" ON call_center_logs;
DROP POLICY IF EXISTS "Allow authenticated changes call_center_logs" ON call_center_logs;
