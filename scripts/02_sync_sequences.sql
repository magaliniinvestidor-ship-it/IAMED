...
-- Usa bigint para tolerar IDs legados de Date.now() (13 dígitos).
-- O parâmetro 'true' em setval marca o valor como já utilizado,
-- então o próximo nextval() retorna max+1.
-- ════════════════════════════════════════════
...
begin
  ...
  select coalesce(max(
    case when id ~ '^presc_[0-9]+$' 
         then substring(id from 'presc_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.prescriptions;
  perform setval('seq_prescriptions', greatest(max_num, 0) + 1, true);

  -- anamnese: anam_NNNN
  select coalesce(max(
    case when id ~ '^anam_[0-9]+$' 
         then substring(id from 'anam_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.anamnese;
  perform setval('seq_anamnese', greatest(max_num, 0) + 1, true);

  -- soap_notes: soap_NNNN
  select coalesce(max(
    case when id ~ '^soap_[0-9]+$' 
         then substring(id from 'soap_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.soap_notes;
  perform setval('seq_soap_notes', greatest(max_num, 0) + 1, true);

  -- diagnoses: diag_NNNN
  select coalesce(max(
    case when id ~ '^diag_[0-9]+$' 
         then substring(id from 'diag_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.diagnoses;
  perform setval('seq_diagnoses', greatest(max_num, 0) + 1, true);

  -- exam_requests: exam_NNNN
  select coalesce(max(
    case when id ~ '^exam_[0-9]+$' 
         then substring(id from 'exam_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.exam_requests;
  perform setval('seq_exam_requests', greatest(max_num, 0) + 1, true);

  -- procedures: proc_NNNN
  select coalesce(max(
    case when id ~ '^proc_[0-9]+$' 
         then substring(id from 'proc_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.procedures;
  perform setval('seq_procedures', greatest(max_num, 0) + 1, true);

  -- clinical_attachments: att_NNNN
  select coalesce(max(
    case when id ~ '^att_[0-9]+$' 
         then substring(id from 'att_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.clinical_attachments;
  perform setval('seq_clinical_attachments', greatest(max_num, 0) + 1, true);

  -- electronic_signatures: sig_NNNN
  select coalesce(max(
    case when id ~ '^sig_[0-9]+$' 
         then substring(id from 'sig_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.electronic_signatures;
  perform setval('seq_electronic_signatures', greatest(max_num, 0) + 1, true);
  ...
  perform setval('seq_aso_exams', greatest(max_num, 0) + 1, true);

  -- patients: PACNNN
  select coalesce(max(
    case when id ~ '^PAC[0-9]+$' 
         then substring(id from '^PAC([0-9]+)$')::bigint 
         else 0 end), 0) into max_num 
  from public.patients;
  perform setval('seq_patients', greatest(max_num, 0) + 1, true);

  -- appointments: CLINNN (também conta agenda_NN legados se existirem)
  select coalesce(max(
    case when id ~ '^CLI[0-9]+$' 
         then substring(id from '^CLI([0-9]+)$')::bigint 
         when id ~ '^agenda_[0-9]+$' 
         then substring(id from 'agenda_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.appointments;
  perform setval('seq_appointments', greatest(max_num, 0) + 1, true);

  -- locations: loc_N
  select coalesce(max(
    case when id ~ '^loc_[0-9]+$' 
         then substring(id from 'loc_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.locations;
  perform setval('seq_locations', greatest(max_num, 0) + 1, true);

  -- clinical_rooms: SALANNN
  select coalesce(max(
    case when id ~ '^SALA[0-9]+$' 
         then substring(id from '^SALA([0-9]+)$')::bigint 
         else 0 end), 0) into max_num 
  from public.clinical_rooms;
  perform setval('seq_clinical_rooms', greatest(max_num, 0) + 1, true);

  -- professionals: PRFNNN
  select coalesce(max(
    case when id ~ '^PRF[0-9]+$' 
         then substring(id from '^PRF([0-9]+)$')::bigint 
         else 0 end), 0) into max_num 
  from public.professionals;
  perform setval('seq_professionals', greatest(max_num, 0) + 1, true);

  -- professional_roles: role_NN
  select coalesce(max(
    case when id ~ '^role_[0-9]+$' 
         then substring(id from 'role_([0-9]+)')::bigint 
         else 0 end), 0) into max_num 
  from public.professional_roles;
  perform setval('seq_professional_roles', greatest(max_num, 0) + 1, true);
